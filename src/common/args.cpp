...
std::optional<OptionsCategory> ArgsManager::GetArgCategory(const std::string& name) const
{
    LOCK(cs_args);
    for (const auto& [category, arg_map] : m_available_args) {
        const auto search = arg_map.find(name);
        if (search != arg_map.end()) {
            return category;
        }
    }
    return std::nullopt;
